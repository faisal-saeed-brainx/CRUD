...
$sql = "SELECT id FROM crud_user WHERE email='". $email ."' AND mode='google'";
$result = $conn->query($sql);

if ($result->num_rows == 0)
{
  $sql = "INSERT INTO crud_user (name, email, role, mode,status,v_code)
  VALUES ('". $name ."', '". $email ."','user','google','inactive','". md5(uniqid(rand(), true)) ."')";

  if ($conn->query($sql) === TRUE) 
  {

    $sql = "SELECT id, role,mode, status, name,v_code FROM crud_user WHERE email='". $email ."' AND mode='google'";
    $result = $conn->query($sql);
...
{
  $sql = "SELECT id, role,mode, status, name FROM crud_user WHERE email='". $email ."' AND mode='google'";
  $result = $conn->query($sql);
